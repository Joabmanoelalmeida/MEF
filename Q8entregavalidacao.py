...
def matrix_jacobiano(n, e, coords):
    ...
    # Linha 1: derivadas em relação a e; Linha 2: derivadas em relação a n
...
def jacobiano_gauss(n_val, e_val, coords):
    ...
    # Criando funções lambda com numpy para avaliação rápida
...
def matrix_Ke_global_elemento(coords, n, e, D, W, gauss_pontos):
    ...
    for pt in gauss_pontos:
        if len(pt) == 3:
            ...
            n_val, e_val = pt
            peso = 1
        # Avalia as funções numericamente, evitando cálculos simbólicos repetidos
        Be_num   = Be_func(n_val, e_val)                # ndarray de tamanho (3, 2*npts)
        detJ_num = detJ_func(n_val, e_val)              # número (float)
...
def deslocamento_reduzido(Ke_reduzido, F_reduzido):
    u_reduced = Ke_reduzido.LUsolve(F_reduzido)
    return u_reduced
...
def tensoes_nos(u_local, n, e, coords, gauss_points, D):
    ...
    # if full 3×3 rule (9 pts), pick only the 4 corners for nodal extrapolation
    if len(t_gauss) > 4:
        # corner indices in 3×3 ordering: 0:(-,-), 2:(-,+), 8:(+,+), 6:(+,-)
...
def matrix_Me_global_elemento(coords, n, e, rho, espessura, W, gauss_pontos):
    # Seleciona funções de forma de acordo com o número de nós (Q4 ou Q8)
    if coords.shape[0] == 4:
    ...
    # Pré-compila as funções N (lambda)
...
#    print("\nMatriz de massa reduzida M_reduzida gauss reduzido:")
 #   sp.pprint(M_reduzida_gauss_reduzido)
    ...
    # --- vetor de forças no formato de setas ---
    # Converte F_global em array (n_nos x 2) [Fx, Fy]
    F_num = np.array([float(F_global[i]) for i in range(F_global.shape[0])]).reshape(n_nos, 2)
    fx, fy = F_num[:, 0], F_num[:, 1]

    # Plotagem em dois subplots: um para Gauss completo e outro para Gauss reduzido
    plt.figure(figsize=(12, 6))
    
    # Plot para Gauss completo
    plt.subplot(1, 2, 1)
    ...
    for elem in elementos:
        ...
        plt.plot(x_d, y_d, 'r-', linewidth=1)
    # plota forças como setas na malha original (setas muito pequenas)
    plt.quiver(orig_nodes[:, 0], orig_nodes[:, 1], fx, fy,
               color='black', angles='xy', scale_units='xy',
               scale=100, width=0.01, label='Forças Nodais')
    plt.xlabel("x")
    plt.ylabel("y")
    ...
    for elem in elementos:
        ...
        y_o = [orig_nodes[i, 1] for i in elem] + [orig_nodes[elem[0], 1]]
        plt.plot(x_o, y_o, 'b--', linewidth=1)
        x_dg = [def_nodes_gauss_reduzido[i, 0] for i in elem] + [def_nodes_gauss_reduzido[elem[0], 0]]
        y_dg = [def_nodes_gauss_reduzido[i, 1] for i in elem] + [def_nodes_gauss_reduzido[elem[0], 1]]
        plt.plot(x_dg, y_dg, 'g-', linewidth=1)
    # também desenha forças (setas muito pequenas) na malha original
    plt.quiver(orig_nodes[:, 0], orig_nodes[:, 1], fx, fy,
               color='black', angles='xy', scale_units='xy',
               scale=100, width=0.01, label='Forças Nodais')
    plt.xlabel("x")
