...
#    sp.pprint(M_global_gauss_reduzido)
 ...
 #   sp.pprint(M_reduzida_gauss_reduzido)
    ...
    for elem in elementos:
        xo = [orig_nodes[i, 0] for i in elem] + [orig_nodes[elem[0], 0]]
        yo = [orig_nodes[i, 1] for i in elem] + [orig_nodes[elem[0], 1]]
        plt.plot(xo, yo, 'b--', linewidth=1)
        xd = [def_nodes[i, 0] for i in elem] + [def_nodes[elem[0], 0]]
        yd = [def_nodes[i, 1] for i in elem] + [def_nodes[elem[0], 1]]
        plt.plot(xd, yd, 'r-',  linewidth=1)
    # Forças nodais
    plt.quiver(orig_nodes[:, 0], orig_nodes[:, 1], fx, fy,
               color='black', angles='xy', scale_units='xy',
               scale=8, width=0.008, label='Forças Nodal')
    ...
    if constr_x:
        plt.quiver(orig_nodes[constr_x, 0], orig_nodes[constr_x, 1],
                   ...
                   color='black', angles='xy', scale_units='xy',
                   scale=2, width=0.02, label='Restrição X')
    ...
    if constr_y:
        plt.quiver(orig_nodes[constr_y, 0], orig_nodes[constr_y, 1],
                   ...
                   color='black', angles='xy', scale_units='xy',
                   scale=2, width=0.02, label='Restrição Y')
    ...
    for elem in elementos:
        ...
        yo = [orig_nodes[i, 1] for i in elem] + [orig_nodes[elem[0], 1]]
        plt.plot(xo, yo, 'b--', linewidth=1)
        xg = [def_nodes_gauss_reduzido[i, 0] for i in elem] + [def_nodes_gauss_reduzido[elem[0], 0]]
        yg = [def_nodes_gauss_reduzido[i, 1] for i in elem] + [def_nodes_gauss_reduzido[elem[0], 1]]
